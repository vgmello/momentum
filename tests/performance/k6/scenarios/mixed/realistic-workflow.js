        ...
        for (let i = 0; i < numInvoices; i++) {
          ...
          if (checkResponse(createResponse, 201, `Create invoice ${i + 1}`)) {
            ...
            if (Math.random() < INVOICE_PAY_PROBABILITY) {
              ...
              http.put(
                endpoints.invoices.markPaid(invoice.invoiceId),
...
function regularUserWorkflow(tenantId, data) {
  group('Regular User Workflow', () => {
    ...
    const cashiersResponse = http.get(
    ...
    group('Create and Pay Invoice', () => {
      ...
      if (checkResponse(createResponse, 201, 'Create invoice')) {
        ...
        const paymentResponse = http.post(
          endpoints.invoices.simulatePayment(invoice.invoiceId),
          JSON.stringify(paymentData),
          {
            headers: headers.withTenant(tenantId),
    ...
    group('Check Invoice Status', () => {
      const invoicesResponse = http.get(
        `${endpoints.invoices.list}?status=Pending`,
        {
          headers: headers.withTenant(tenantId),
...
function adminUserWorkflow(tenantId, data) {
  group('Admin User Workflow', () => {
    ...
    group('Invoice Review', () => {
      ...
      const overdueResponse = http.get(
        `${endpoints.invoices.list}?status=Pending&dueBefore=${today}`,
        {
          headers: headers.withTenant(tenantId),
    ...
    group('View Cashiers', () => {
      ...
      if (checkResponse(listResponse, 200, 'List cashiers')) {
        ...
        if (cashiers && cashiers.length > 0) {
          ...
          http.get(
            endpoints.cashiers.get(randomCashier.cashierId),
            {
              headers: headers.withTenant(tenantId),
    ...
    group('View Invoices', () => {
      ...
      if (checkResponse(listResponse, 200, 'List invoices')) {
        ...
        if (invoices && invoices.length > 0) {
          ...
          http.get(
            endpoints.invoices.get(randomInvoice.invoiceId),
            {
              headers: headers.withTenant(tenantId),
      ...
      http.get(
        `${endpoints.invoices.list}?status=${randomStatus}`,
        {
          headers: headers.withTenant(tenantId),
