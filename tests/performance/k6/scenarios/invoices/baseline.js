...
export function setup() {
    ...
    const cashierResponse = http.post(endpoints.cashiers.create, JSON.stringify(cashierData), { headers: headers.withTenant(tenantId) });
...
export default function main(data) {
    ...
    group("Invoices CRUD Operations", () => {
        ...
        if (invoiceId) {
            group("Get Invoice by ID", () => {
                const getResponse = http.get(endpoints.invoices.get(invoiceId), {
                    headers: headers.withTenant(tenantId),
        ...
        if (invoiceId && invoiceVersion) {
            group("Mark Invoice as Paid", () => {
                ...
                const markPaidResponse = http.put(endpoints.invoices.markPaid(invoiceId), JSON.stringify(markPaidData), {
                    headers: headers.withTenant(tenantId),
    ...
    group("Invoice Lifecycle", () => {
        ...
        if (createResponse.status === 201) {
            ...
            const cancelResponse = http.put(endpoints.invoices.cancel(invoice.invoiceId), JSON.stringify(cancelData), {
                headers: headers.withTenant(tenantId),
    ...
    group("Invoices Search and Filter", () => {
        ...
        const searchResponse = http.get(`${endpoints.invoices.list}?${queryString}`, {
            headers: headers.withTenant(tenantId),
